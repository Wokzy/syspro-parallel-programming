...
Для формальности определим интервал `X(x1, x2)` из лекций как пару элементов таких, что `x1 -> x2`:
- `X(x1, x2)`. От обратного: `x1 -> x2` по определению интервала, но по предположению `x2 -> x1` => противоречие с условием строгого порядка над событиями

- `X(x1, x2), Y(y1, y2)`.` x1 -> x2 -> y1 -> y2`. По транзитивности `x1 -> y2`, но тогда по свойству антисимметричности событий `!y2 -> x1`, а тогда `!Y -> X`.

- `X(x1, x2), Y(y1, y2), Z(z1, z2)`. `x1 -> x2 -> y1 -> y2 -> z1 -> z2` по условию. По транзитивности `x2 -> z1`. А тогда по определению `X -> Z` ЧТД.

- Пример: пусть `x1 -> y1 -> x2 -> y2`, тогда `X(x1, x2) != Y(y1, y2)`. При этом `!(x2 -> y1)` и `!(y2 -> x1)`, а потому `X -> Y` false and `Y -> X` is also false


#### Доказательство для интервалов на счётном множестве
